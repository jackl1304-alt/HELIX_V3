...
var regulatoryUpdates = pgTable("regulatory_updates", {
  ...
  // Normalisierte Titel-Hash für schnelle Dedup
  description: text("description"),
  ...
  // True wenn aus Primärquelle direkt übernommen
  ...
  // True falls echte behördliche/amtliche Kostendaten zugeordnet
...
var costItems = pgTable("cost_items", {
  ...
  // Referenz auf Behörde/Institution
  feeType: varchar("fee_type"),
  // application_fee, renewal_fee, inspection_fee, etc.
  description: text("description"),
  // Amtliche Beschreibung oder exakter Gebührenzweck
...
var normativeActions = pgTable("normative_actions", {
  ...
  // Kurzcode für interne Verlinkung
  actionDescription: text("action_description"),
  // Genaue Maßnahme (quellbasiert, nicht generativ)
  requiredDocuments: jsonb("required_documents"),
...
var normChanges = pgTable("norm_changes", {
  ...
  // konstanter Identifier über Revisionen
...
async function main() {
  ...
  console.log(`\u{1F50D} Lade Health Canada Health Product Recalls: ${url}`);
  const res = await fetch(url, { headers: { "User-Agent": "HelixRegBot/1.0" } });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  const json2 = await res.json();
  const items = json2.results?.HEALTH || [];
  console.log(`\u{1F4E6} Empfangen: ${items.length} Eintr\xE4ge`);
  let inserted = 0;
  for (const item of items.slice(0, limit)) {
    const title = item.title;
    const link = `https://healthycanadians.gc.ca${item.url}`;
    const datePublished = new Date(item.date_published * 1e3);
    const hashedTitle = crypto.createHash("sha256").update(title.toLowerCase()).digest("hex");
    const dup = await sql2`SELECT id FROM regulatory_updates WHERE hashed_title=${hashedTitle} AND source_url=${link}`;
    if (dup.length) {
      console.log(`\u23ED\uFE0F Duplikat \xFCbersprungen: ${title.slice(0, 60)}`);
      continue;
    }
    try {
      ...
      console.error(`Health Canada Insert Fehler: ${title.slice(0, 60)} -> ${e2.message}`);
    }
  }
  console.log(`\u2705 Health Canada neu eingef\xFCgt: ${inserted}`);
}
main().catch((e2) => {
  console.error(e2);
  process.exit(1);
