...
async function fetchFirstValidFeed() {
  for (const url of FEEDS) {
    try {
      ...
      if (text2.includes("<rss")) return { xml: text2, url };
    } catch {
    }
  }
...
async function main() {
  ...
  const parsed = await (0, import_xml2js.parseStringPromise)(xml);
  ...
  for (const it of items.slice(0, 40)) {
    ...
    try {
      ...
      console.error("WHO Insert Fehler:", e2.message);
    }
  }
  console.log("WHO neu eingef\xFCgt:", inserted);
}
main().catch((e2) => {
  console.error(e2);
  process.exit(1);
