...
export function registerRoutes(app: Express) {
  ...
  app.get('/api/dashboard/stats', async (req, res) => {
    try {
      ...
      res.json({
        ...
        generatedAt: new Date().toISOString(),
        fallback: true,
        message: 'Database unavailable, returning fallback dashboard statistics'
      });
    }
  });
  ...
  app.get('/api/data-sources', async (req, res) => {
    try {
      ...
      if (enhancedSources.length < 20) {
        console.warn(`[API] Data sources count low (${enhancedSources.length}); supplementing with fallback catalog`);
        try {
          const fallbackDataSources = await dbStorage.getDataSources();
          const safeFallback = Array.isArray(fallbackDataSources) ? fallbackDataSources : [];
          const finalFallback = safeFallback.map(source => {
            const isActive = source.is_active ?? source.isActive ?? false;
            const lastSync = source.last_sync_at ?? source.lastSync ?? null;
            return {
              ...source,
          ...
          if (finalFallback.length > enhancedSources.length) {
            console.log(`[API] Returning supplemented data sources: ${finalFallback.length} sources`);
            return res.json(finalFallback);
          }
        } catch (fallbackError) {
          console.error('[API] Data source supplement failed:', fallbackError);
        }
      }
      ...
      // FALLBACK: Versuche explizit den Speicher-Fallback zu verwenden
      try {
        const fallbackDataSources = await dbStorage.getDataSources();
        const safeFallback = Array.isArray(fallbackDataSources) ? fallbackDataSources : [];
        const enhancedFallback = safeFallback.map(source => {
          const isActive = source.is_active ?? source.isActive ?? false;
          const lastSync = source.last_sync_at ?? source.lastSync ?? null;
          return {
            ...source,
        ...
        if (enhancedFallback.length > 0) {
          console.log(`[API] Returning fallback data sources: ${enhancedFallback.length} sources`);
          return res.json(enhancedFallback);
        }
      } catch (fallbackError) {
        console.error('[API] Data source fallback failed:', fallbackError);
      }

      // Minimal fallback only as a final option
      res.json([
  ...
  // Master-catalog routes must be mounted BEFORE the legacy /api/patents
  // handler so the curated real-data endpoint wins on the same path.
  // Express's app.use falls through unmatched paths to the next middleware,
  // so /api/patents/sync and /api/patents/stats still reach the legacy router.
  app.use('/api', masterCatalogRoutes);

  // Patents routes for global patent data
  app.use('/api/patents', patentsRoutes);
  console.log('✅ Patents routes loaded successfully');

  // Master Sources Catalog (ISO 13485 / QMSR — master_sources_formatted.docx)
  app.use('/api', masterSourcesRoutes);
  console.log('✅ Master sources catalog routes loaded successfully');

  // Formular-Assistent — legally-compliant regulatory form templates + drafts.
  app.use('/api', formTemplatesRoutes);
  console.log('✅ Form templates routes loaded successfully');
