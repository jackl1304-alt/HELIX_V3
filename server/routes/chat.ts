// Chat Board API Routes für Tenant-Administrator-Kommunikation + RAG Chat
...
// Validation schema for RAG chat requests
...
// ============================================
// RAG-based Regulatory Intelligence Chat
// ============================================

/**
 * POST /api/chat/rag - Process regulatory query through RAG + Agent Router
 */
router.post('/rag', async (req: Request, res: Response) => {
  try {
    ...
    logger.info('RAG chat request received', { queryLength: query.length });

    // Process through RAG pipeline with agent routing
    const response = await processRegulatoryQuery(query);
...
/**
 * GET /api/chat/rag/health - Health check for RAG chat service
 */
router.get('/rag/health', (req: Request, res: Response) => {
  res.json({
    status: 'healthy',
    service: 'chat-rag-agent-router',
